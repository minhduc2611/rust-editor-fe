...
import { startWorkers } from "./snippets/wasm-bindgen-rayon-7afa899f36665473/src/workerHelpers.tsx";
let wasm;
...
export class WorldState {
  ...
  references(line_number, column, include_declaration) {
    const ret = wasm.worldstate_references(
      this.ptr,
      line_number,
      column,
      include_declaration
  ...
  rename(line_number, column, new_name) {
    ...
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.worldstate_rename(
      this.ptr,
      line_number,
      column,
      ptr0,
  ...
  type_formatting(line_number, column, ch) {
    const ret = wasm.worldstate_type_formatting(
      this.ptr,
      line_number,
      column,
      ch.codePointAt(0)
  ...
  goto_implementation(line_number, column) {
    const ret = wasm.worldstate_goto_implementation(
      this.ptr,
      line_number,
      column
    );
...
function initMemory(imports, maybe_memory) {
  imports.wbg.memory =
    maybe_memory ||
    new WebAssembly.Memory({ initial: 36, maximum: 16384, shared: true });
}
